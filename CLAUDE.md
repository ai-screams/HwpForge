...
- Known still-deferred: richer strike/underline line families; Wave 5 gap C (masterPage carry — macOS 한컴 fixture 비대칭, PC 한컴 fixture 대기, task #33); multi-section / page-border-fill (fixture 미작성); 한컴-authored .hwpx의 multi-run span 디코딩 (편집 알고리즘 prerequisite, task #96); **#120 byte-level diff trigger** — Wave 12p/12q 다 적용해도 SUMMERY 필드 변환 시 한컴 "낮은 보안 수준 복구" 경고 + 첫 열기 시 빈 placeholder 잔존 (저장 한 번이면 자동 update 되므로 functional impact 작음, byte-level diff 진단은 후속 wave)
