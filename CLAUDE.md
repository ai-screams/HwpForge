...
- Known still-deferred: richer strike/underline line families; Wave 5 gap C (masterPage carry — macOS 한컴 fixture 비대칭, PC 한컴 fixture 대기, task #33); multi-section / page-border-fill (fixture 미작성); 한컴-authored .hwpx의 multi-run span 디코딩 (편집 알고리즘 prerequisite, task #96); **#120 byte-level diff trigger** — Wave 12p/12q 다 적용해도 SUMMERY 필드 변환 시 한컴 "낮은 보안 수준 복구" 경고 + 첫 열기 시 빈 placeholder 잔존 (저장 한 번이면 자동 update 되므로 functional impact 작음, byte-level diff 진단은 후속 wave); **Wave 12p Step 5 (#134)** — HWP5 변환 path 의 cross-ref `?` 표시 (HWP5 wire 가 target element instance ID carry 안 함, projection 단 pre-scan + ID synthesis 알고리즘 필요)

**Workspace Facts (code-grounded, 2026-06-09 Wave 12q)**:
...
- Tracked Rust `src` files under `crates/`: `148`
- Workspace nextest count: `2,463` (passed) + `2` skipped (Wave 12p 신규 게이트들 누적)
