...
- **Phase 12 HWP5 carry series complete** (active branch `feat/phase12-hwp5-gso-shapes`, 21 commits ahead of `main`, 2026-06-02):
  ...
  - Wave 12n Step 6 (latest, 2026-06-06): `Control::PathField` lossless HWPX carry — encoder가 `<hp:fieldBegin type="PATH" fieldid="628121972" editable="0">` + `Format=` param native wire 직접 emit (이전 LOSSY SUMMERY surrogate 대체), decoder가 `type="PATH"` 인식 + `Format` 우선 파싱 → `PathFieldCommand::from_wire`. #120 (한컴 "낮은 보안수준 복구" 경고의 가장 강한 트리거) 핵심 해소. end-to-end 검증: `sample-field-docsummary.hwp` 변환 결과 PATH 필드가 한컴 native HWPX 와 byte-identical (id 카운터만 차이).
- Known still-deferred: richer strike/underline line families; Wave 5 gap C (masterPage carry — macOS 한컴 fixture 비대칭, PC 한컴 fixture 대기, task #33); multi-section / page-border-fill (fixture 미작성); 한컴-authored .hwpx의 multi-run span 디코딩 (편집 알고리즘 prerequisite, task #96); Wave 12n leftover (outline level off-by-1 #121, hp10 fallback #122, linesegarray drop #123, editable bit propagation #124 — 한컴 보안 경고 보조 트리거, Step 6 단독 fix 후 잔존)
...
- Workspace nextest count: `2,445` (passed) + `2` skipped (Wave 12n Step 6: +3 신규 PATH 게이트, -2 폐기 LOSSY 게이트, 순 +1)
