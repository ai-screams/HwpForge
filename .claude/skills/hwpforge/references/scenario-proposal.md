...
> **참고**: `government` 프리셋은 `presets.rs`의 4종 카탈로그(`default`/`modern`/`classic`/`latest`)에 없습니다 — CLI `convert --preset`는 이 네 카탈로그 이름을 모두 받아들이므로, 문서 성격에 맞는 이름을 고르면 됩니다. 자세한 내용은 [templates.md](templates.md).
