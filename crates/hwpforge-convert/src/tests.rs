//! Integration-style tests for the `hwpforge-convert` crate entry points
...
use hwpforge_smithy_hwp5::schema::header::{Hwp5RawCharShape, Hwp5RawFaceName};
use hwpforge_smithy_hwp5::style_store::Hwp5StyleStore;
use hwpforge_smithy_hwp5::{DocInfoResult, Hwp5Warning};
use hwpforge_smithy_hwpx::{HwpxDecoder, PackageReader};

use crate::{hwp5_to_hwpx, hwp5_to_hwpx_bytes};

fn default_test_char_shape() -> Hwp5RawCharShape {
    Hwp5RawCharShape {
        font_ids: [0; 7],
        font_ratios: [100; 7],
        font_spacings: [0; 7],
...
fn doc_info_with_style_projection_warning() -> DocInfoResult {
...
struct ImageFixtureExpectation {
    ...
    // Census-only expectations retained for fixture documentation parity with
    // the HWP5-side census test; the conversion test asserts only the
    // post-convert layout fields below.
    #[allow(dead_code)]
...
fn fixture_path(name: &str) -> PathBuf {
    ...
    // basename fallback: walk to find matching filename
    let basename = std::ffi::OsStr::new(
        Path::new(name).file_name().unwrap_or_else(|| std::ffi::OsStr::new(name)),
    );
    let mut found: Vec<PathBuf> = Vec::new();
    fn walk(dir: &Path, target: &std::ffi::OsStr, acc: &mut Vec<PathBuf>) {
    ...
    walk(&root, basename, &mut found);
    found.into_iter().next().unwrap_or(direct)
}
...
fn read_section_xml(path: &Path, index: usize) -> String {
    ...
    let mut package = PackageReader::new(&bytes).expect("converted hwpx should open as package");
...
fn inspect_summary_counts_style_projection_warnings() {
    ...
    let (_hwpx_store, style_warnings) = crate::hwp5_style_store_to_hwpx(&store);
    assert!(
...
fn collect_decoded_image_layout(
    ...
    for section in decoded.document.sections() {
        count_images_in_paragraphs(&section.paragraphs, DecodedImageLocation::Body, &mut layout);
...
fn count_images_in_run(run: &Run, location: DecodedImageLocation, layout: &mut DecodedImageLayout) {
    match &run.content {
...
fn count_images_in_table(table: &Table, layout: &mut DecodedImageLayout) {
    for row in &table.rows {
        for cell in &row.cells {
            count_images_in_paragraphs(&cell.paragraphs, DecodedImageLocation::TableCell, layout);
        }
    }
}
...
fn hwp5_to_hwpx_full_report_keeps_leading_image_non_zero() {
    ...
    let first_body_image =
        first_image_in_paragraphs(&section0.paragraphs).expect("section 0 should contain an image");
...
fn hwp5_to_hwpx_image_fixture_matrix_emits_valid_hwpx_packages() {
    ...
    for case in cases {
        ...
        let total_tables: usize =
            decoded.document.sections().iter().map(|section| section.content_counts().tables).sum();
        assert_eq!(total_tables, case.expected_table_count_after_convert, "fixture={}", case.name);
        assert_eq!(
...
fn hwp5_to_hwpx_user_sample_indexmark_multi_preserves_order_and_secondary_keys() {
    ...
    {
        let needle =
            format!("<hp:firstKey>{primary}</hp:firstKey><hp:secondKey>{secondary}</hp:secondKey>");
        assert!(
            section_xml.contains(&needle),
...
fn hwp5_to_hwpx_user_sample_fwspace_carries_fixed_width_space() {
    ...
    assert!(warnings.is_empty(), "fwspace fixture should convert without warnings: {warnings:?}",);
...
fn hwp5_to_hwpx_user_sample_tab_preserves_inline_tab_text_and_custom_tab_def() {
    ...
    let warnings = hwp5_to_hwpx(&source, &out).expect("user sample tab conversion should succeed");
...
fn hwp5_to_hwpx_user_sample_checkable_multiline_preserves_item_state_and_continuation() {
    ...
    let continuation = paragraphs
        .iter()
        .find(|paragraph| paragraph.text_content().contains("second paragraph of the same item"))
    ...
    let continuation_shape = decoded.style_store.para_shape(continuation.para_shape_id).unwrap();
...
fn hwp5_to_hwpx_user_sample_numbered_list_preserves_numbering_semantics() {
    ...
    let warnings =
        hwp5_to_hwpx(&source, &out).expect("user sample numbered list conversion should succeed");
...
fn hwp5_to_hwpx_user_sample_hyperlink_preserves_field_control_and_surrounding_text() {
    ...
    assert!(
        section_xml.contains(r#"<hp:stringParam name="Path">https://openai.com</hp:stringParam>"#,),
...
fn hwp5_to_hwpx_user_sample_footnote_carries_four_footnotes() {
    ...
    assert!(warnings.is_empty(), "footnote fixture should convert without warnings: {warnings:?}");
    ...
    let section_xml = read_section_xml(&out, 0);
    let footnote_element_count =
        section_xml.matches("<hp:footNote ").count() + section_xml.matches("<hp:footNote>").count();
    assert_eq!(footnote_element_count, 4, "converted hwpx must emit four <hp:footNote> elements");
...
fn hwp5_to_hwpx_user_sample_bookmark_crossref_preserves_controls() {
    ...
    assert!(
        section_xml.contains(r#"<hp:stringParam name="RefType">TARGET_BOOKMARK</hp:stringParam>"#),
...
fn hwp5_to_hwpx_user_sample_crossref_emits_nonzero_fieldid() {
    ...
    let begin_pos = section_xml.find(begin_marker).expect("section xml must contain a fieldBegin");
    ...
    let extract_fieldid = |tag: &str| -> String {
        ...
        let start = tag.find(needle).expect("tag must carry a fieldid attribute") + needle.len();
    ...
    let begin_id: i64 =
        extract_attr(begin_tag, "id").parse().expect("CROSSREF fieldBegin id must be an integer");
...
fn hwp5_to_hwpx_user_sample_page_number_preserves_section_page_number() {
    ...
    let section_xml = read_section_xml(&out, 0);
    assert!(
        section_xml.contains(r#"<hp:pageNum pos="BOTTOM_CENTER" formatType="DIGIT" sideChar="-""#),
...
fn hwp5_to_hwpx_user_sample_multilevel_list_projects_as_outline_levels() {
    ...
    assert!(warnings.is_empty(), "multilevel numbering fixture should convert without warnings");
    ...
    let outline_levels: Vec<_> = headings
        ...
        .collect();
    assert!(outline_levels.len() >= 2, "fixture should project at least two outline paragraphs");
...
fn hwp5_to_hwpx_user_sample_underline_variants_preserves_all_shapes() {
    ...
    for (i, expected) in expected_shapes.iter().enumerate() {
        ...
        let cs = decoded.style_store.char_shape(run.char_shape_id).expect("char shape must exist");
        assert_eq!(
...
fn hwp5_to_hwpx_user_sample_strike_variants_preserves_line_family() {
    ...
    let warnings = hwp5_to_hwpx(&source, &out).expect("strike variants conversion should succeed");
...
fn hwp5_to_hwpx_user_sample_breakwordlatin_variants_preserves_hyphenation() {
    ...
    let shape_19 =
        decoded.style_store.para_shape(ParaShapeIndex::new(19)).expect("para shape 19 must exist");
    assert_eq!(shape_19.break_latin_word, WordBreakType::KeepWord);

    let shape_20 =
        decoded.style_store.para_shape(ParaShapeIndex::new(20)).expect("para shape 20 must exist");
...
fn hwp5_to_hwpx_user_sample_line_spacing_preserves_all_modes() {
    ...
    let shape_0 =
        decoded.style_store.para_shape(ParaShapeIndex::new(0)).expect("para shape 0 must exist");
    ...
    let shape_21 =
        decoded.style_store.para_shape(ParaShapeIndex::new(21)).expect("para shape 21 must exist");
    assert_eq!(
...
fn hwp5_to_hwpx_user_sample_page_break_preserves_break_and_keep_flags() {
    ...
    let s22 = decoded.style_store.para_shape(ParaShapeIndex::new(22)).expect("para shape 22");
    assert!(s22.keep_lines_together, "para 22 (같은 쪽에 두기) expects keep_lines_together = true");
...
fn hwp5_to_hwpx_user_sample_border_shading_carries_border_fill_per_paragraph() {
    ...
    let _warnings = hwp5_to_hwpx(&source, &out).expect("border-shading conversion should succeed");
...
fn hwp5_to_hwpx_chart_fixture_emits_embedded_chart_switch_block() {
    ...
    assert_eq!(&ole_bytes[..4], b"\xD0\xCF\x11\xE0", "BinData/ole1.ole must start with OLE2 magic");
