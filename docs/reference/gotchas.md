...
````admonish danger title="7. 선(Line) 도형의 좌표는 hc: 네임스페이스"
...
````admonish danger title="14. 다각형 꼭짓점도 hc: 네임스페이스"
...
````admonish warning title="4. TextBox는 control 요소가 아님"
...
````admonish warning title="9. 수식(Equation)에는 shape common 블록이 없음"
...
````admonish danger title="10. 차트 XML을 content.hpf manifest에 등록하면 안 됨"
...
````admonish danger title="11. 차트 데이터에 <c:f> formula 참조가 필수"
...
````admonish danger title="12. 차트 시리즈 이름 <c:tx>는 직접값만 허용"
...
````admonish danger title="13. <hp:chart> 요소에 dropcapstyle 속성 필수"
...
```admonish warning title="16. TextBox(hp:rect) 인코딩 6가지 핵심 규칙"
...
````admonish warning title="17. 다각형 꼭짓점은 첫 번째를 마지막에 반복해야 닫힘"
...
````admonish warning title="23. VHLC/VOHLC 주식 차트는 4축 combo layout 필수"
...
````admonish danger title="3. landscape 속성값이 스펙과 반대"
...
```admonish warning title="24. Modern 스타일셋의 개요 8/9/10 paraPr 인덱스는 비순차"
...
````admonish danger title="26. MasterPage XML은 prefix 없는 루트 + 15개 xmlns 전체 선언 필수"
...
````admonish warning title="8. paraPr 당 switch가 여러 개일 수 있음"
...
````admonish warning title="21. 하이퍼링크는 fieldBegin/fieldEnd 패턴 (<hp:hyperlink> 없음)"
...
````admonish warning title="22. 각주/미주는 같은 문단의 인라인 Run으로 삽입"
...
````admonish danger title="27. 날짜/시간 필드는 type="SUMMERY" (오타 주의)"
...
````admonish warning title="28. 본문 쪽번호는 <hp:autoNum> 사용 (fieldBegin 아님)"
...
````admonish warning title="29. page_break는 문단 속성으로 직접 인코딩"
...
````admonish warning title="20. breakNonLatinWord는 반드시 KEEP_WORD"
...
````admonish warning title="30. DropCapStyle은 PascalCase (도형 레벨 속성)"
...
```admonish info title="2. HWP5 TagID에 +16 오프셋"
...
```admonish info title="5. Foundation 의존성은 최소화"
...
````admonish info title="18. schemars 1.x: schema_name() 반환 타입 변경"
...
````admonish info title="19. quick-xml 0.39: unescape() 제거"
